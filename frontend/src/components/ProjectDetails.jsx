        ...
        <header className="mt-3 border-b border-slate-300 pb-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <span className="grid size-12 place-items-center rounded-xl bg-[#173b35] text-[#dce993]"><FolderKanban aria-hidden="true" className="size-6" /></span>
              <p className="mt-5 text-[11px] font-bold tracking-[0.14em] text-emerald-800 uppercase">Project details</p>
              <h1 className="mt-1 break-words text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">{project.name}</h1>
            </div>
            {project.is_owner ? <button type="button" onClick={openMemberDialog} className="flex min-h-11 w-full cursor-pointer items-center justify-center gap-2 rounded-lg bg-[#173b35] px-5 text-sm font-bold text-white shadow-sm hover:bg-[#204b43] focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-emerald-800 sm:w-auto"><Plus aria-hidden="true" className="size-4" />Add member</button> : null}
          </div>
        </header>

        <div className="grid gap-5 py-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
          <section aria-labelledby="overview-heading" className="bg-white p-5 shadow-[0_1px_3px_rgba(15,23,42,0.08)] ring-1 ring-slate-200 sm:p-6">
            <h2 id="overview-heading" className="text-lg font-semibold">Overview</h2>
            <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-600">{project.description || "No description has been added to this project."}</p>
            <dl className="mt-7 grid gap-4 border-t border-slate-200 pt-5 sm:grid-cols-2">
              <div><dt className="flex items-center gap-2 text-xs font-bold tracking-wide text-slate-500 uppercase"><CalendarClock aria-hidden="true" className="size-4" />Created</dt><dd className="mt-1.5 text-sm font-semibold">{DATE_FORMATTER.format(new Date(project.created_at))}</dd></div>
              <div><dt className="flex items-center gap-2 text-xs font-bold tracking-wide text-slate-500 uppercase"><Users aria-hidden="true" className="size-4" />People</dt><dd className="mt-1.5 text-sm font-semibold">{people.length} {people.length === 1 ? "person" : "people"}</dd></div>
            </dl>
          </section>

          <section aria-labelledby="members-heading" className="bg-white shadow-[0_1px_3px_rgba(15,23,42,0.08)] ring-1 ring-slate-200">
            <div className="flex min-h-12 items-center justify-between border-b border-slate-200 px-4"><h2 id="members-heading" className="flex items-center gap-2 font-semibold"><Users aria-hidden="true" className="size-5 text-emerald-800" />Project people</h2><span className="text-xs font-semibold text-slate-500">{people.length}</span></div>
